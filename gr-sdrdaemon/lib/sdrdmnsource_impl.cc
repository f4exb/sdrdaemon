...
sdrdmn_source::sptr sdrdmn_source::make(std::size_t itemsize,
		...
		bool eof)
{
	return gnuradio::get_initial_sptr(new sdrdmn_source_impl(itemsize, ipaddr, port, payload_size, eof));
}

sdrdmn_source_impl::sdrdmn_source_impl(size_t itemsize,
...
sdrdmn_source_impl::~sdrdmn_source_impl()
{
	if (d_connected)
	{
		disconnect();
...
void sdrdmn_source_impl::connect(const std::string &host, int port)
{
	if (d_connected)
	{
		disconnect();
	...
	if (host.size() > 0)
	{
		...
		d_udp_thread = gr::thread::thread(
				boost::bind(&sdrdmn_source_impl::run_io_service, this));
		d_connected = true;
	}
}

void sdrdmn_source_impl::disconnect()
{
...
int sdrdmn_source_impl::get_port(void)
{
...
void sdrdmn_source_impl::start_receive()
{
	d_socket->async_receive_from(
			...
			d_endpoint_rcvd,
			boost::bind(
					&sdrdmn_source_impl::handle_read,
					this,
...
void sdrdmn_source_impl::handle_read(const boost::system::error_code& error, std::size_t bytes_transferred)
{
	if (!error)
	{
...
int sdrdmn_source_impl::work(int noutput_items,
