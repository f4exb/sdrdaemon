  ...
  namespace sdrdaemon {
                     ...
                     int payload_size, bool eof)
    {
      return gnuradio::get_initial_sptr
        (new sdrdmn_source_impl(itemsize, ipaddr, port,
                                payload_size, eof));
    }
    ...
    void sdrdmn_source_impl::connect(const std::string &host, int port)
    {
      ...
      if(host.size() > 0) {
        ...
        start_receive();
        d_udp_thread = gr::thread::thread(boost::bind(&sdrdmn_source_impl::run_io_service, this));
        d_connected = true;
      }
    }
    ...
    void sdrdmn_source_impl::start_receive()
    {
      d_socket->async_receive_from(boost::asio::buffer((void*)d_rxbuf, d_payload_size), d_endpoint_rcvd,
                                   boost::bind(&sdrdmn_source_impl::handle_read, this,
                                               boost::asio::placeholders::error,
                                               boost::asio::placeholders::bytes_transferred));
    }

    void sdrdmn_source_impl::handle_read(const boost::system::error_code& error,
